...
Αν ο χρήστης συνδέεται του μεφανίζει τα νέα του μηνύματα.
Αν ο χρήστης εγγράφεται του εμφανίζει κάποιες οδηγίες ως προς το πώς να χρησιμοποιήσει την εφαρμογή.  
Βγάζει το κεντρικό μενού επιλογών στον χρήστη : 1.Στέλνει μήνυμα 2. Κάνει like 3. Κάνει dislike  4. Απαντάει σε ένα άλλο μήνυμα 5. Φεύγει από την εφαρμογή 6.Βλέπει τις οδηγίες.  
Αν ο χρήστης πατήσει 2,3 ή 4 ελέγχεται μέσω της findLastMessage άμα υπάρχει το συγκεκριμένο μήνυμα.  
Όταν ο χρήστης επιθυμήσει να φύγει από την CodeCom, εισάγεται στον πίνακα το τελυταίο μήνυμα που υπάρχει εκείνη τη στιγμή στη βάση ως το τελευταίο μήνυμα που έχει δει.  
##### Κλάση Message:  
...
6. updateMessages(String username, String answer, int type) :  
    1. Κάνει update τον πίνακα Messages  
        ...
        + type == -2 : το μήνυμα ειναι dislike  
